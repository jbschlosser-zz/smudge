...
START_TEST(test_line_buffer_add_data)
{
    ...
    ck_assert(memcmp(line_buffer_get_line_relative_to_current(buf, 0)->_data, expected_line0->_data, sizeof(mud_char_t) * mud_string_length(expected_line0)) == 0);
    mud_string *expected_line1 = mud_string_create_from_c_string(256, "a");
    ck_assert(memcmp(line_buffer_get_line_relative_to_current(buf, 1)->_data, expected_line1->_data, sizeof(mud_char_t) * mud_string_length(expected_line1)) == 0);
    mud_string *expected_line2 = mud_string_create_from_c_string(256, "is");
    ck_assert(memcmp(line_buffer_get_line_relative_to_current(buf, 2)->_data, expected_line2->_data, sizeof(mud_char_t) * mud_string_length(expected_line2)) == 0);
    mud_string *expected_line3 = mud_string_create_from_c_string(256, "this");
    ck_assert(memcmp(line_buffer_get_line_relative_to_current(buf, 3)->_data, expected_line3->_data, sizeof(mud_char_t) * mud_string_length(expected_line3)) == 0);
