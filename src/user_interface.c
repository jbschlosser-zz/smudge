...
static void populate_window_with_scrollback(mud_window *mwin, line_buffer *output_data, int scrollback_index)
{
    ...
    if(scrollback_index >= line_buffer_num_lines(output_data) - win_size_lines) {
        ...
        for(j = i; j >= scrollback_index; --j) {
            ...
            mud_window_write_text(mwin, line->_data + line_start_index, color_string_length(line) - line_start_index);
...
void user_interface_refresh_input_line_window(user_interface *ui, input_line *input)
{
    ...
    mud_window_write_text(ui->_input_line_window, color_string_get_data(contents), color_string_length(contents));
